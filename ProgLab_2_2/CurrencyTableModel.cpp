...
QVariant CurrencyTableModel::data(const QModelIndex &idx, int role) const {
    ...
    switch (role) {
        case Qt::DisplayRole: {
            switch (idx.column()) {
                case 0: {
                    value = this->_rate_.at(idx.row()).get_NumCode();
                    break;
                }
                case 1: {
                    value = this->_rate_.at(idx.row()).get_CharCode();
                    break;
                }
                case 2: {
                    value = this->_rate_.at(idx.row()).get_Nominal();
                    break;
                }
                case 3: {
                    value = this->_rate_.at(idx.row()).get_Name();
                    break;
                }
                case 4: {
                    value = this->_rate_.at(idx.row()).get_Value();
                    break;
                }
                case 5: {
                    if (this->_rate_.at(idx.row()).alteration() > 0) {
                        value =
                            "+" + QString::number(
                                      this->_rate_.at(idx.row()).alteration());
                    } else {
                        value = this->_rate_.at(idx.row()).alteration();
                    }
                    break;
                }
                case 6: {
...
void CurrencyTableModel::populate(CurrencyRate new_rate) {
    int idx = this->_rate_.size();
    this->beginInsertRows(QModelIndex(), 1, idx);
    this->_rate_ = new_rate;
